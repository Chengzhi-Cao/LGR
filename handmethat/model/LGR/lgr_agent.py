...
from models.lgr.lgr import LGRNetwork
...
class LGRAgent:
    def __init__(
        ...
        self.target_network = LGRNetwork(
            tb=tb,
            log=log,
            vocab_size=len(self.tokenizer),
            envs=envs,
    ...
    def act(self, states, poss_acts, poss_act_strs, p_rules , sample=True):
        """
        Parameters
        ----------
        poss_acts: [
        ...
        idxs, qvals = self.network.act(states, poss_acts, poss_act_strs, p_rules)

        # Get the sampled action for each environment
        act_ids = [poss_acts[batch][idx] for batch, idx in enumerate(idxs)]
        return act_ids, idxs, qvals

    def act_topk(self, states, poss_acts, rules):
        """
        """
        idxs = self.network.act_topk(states, poss_acts, rules)
