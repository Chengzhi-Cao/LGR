...
class LGRNetwork(nn.Module):
    def __init__(self,
        ...
        super(LGRNetwork, self).__init__()
        ...
        lgr.init_model(self, args, vocab_size, tokenizer)
    ...
    def forward(self, state_batch, act_batch):
        ...
        act_out = lgr.packed_rnn(self, act_batch, self.act_encoder)
        # Encode the various aspects of the state
        obs_out = lgr.packed_rnn(self, state.obs, self.obs_encoder)
        look_out = lgr.packed_rnn(self, state.description, self.look_encoder)
        inv_out = lgr.packed_rnn(self, state.inventory, self.inv_encoder)
        
        rule_out = lgr.packed_rnn(self, self.rule_variable, self.rule_encoder)
        state_out = torch.cat((obs_out, look_out, inv_out, rule_out), dim=1)
        ...
        lgr_scores = self.act_scorer(z).squeeze(-1)

        # Split up the q-values by batch
        return lgr_scores.split(act_sizes)
    ...
    def act(
        ...
        return lgr.act(self, states, valid_ids, valid_strs, self.log, rules, graph_masks)
