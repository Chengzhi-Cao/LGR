...
from lgr import LGRNetwork

device = torch.device("cuda" if torch.cuda.is_available() else "cpu")


class LGRInvDynQNetwork(LGRNetwork):
    ...
    def __init__(
        ...
        self.state_encoder = nn.Linear(
            3 * args.hidden_dim + (1 if self.augment_state_with_score else 0), args.hidden_dim)
        self.inverse_dynamics = nn.Sequential(nn.Linear(
            2 * args.hidden_dim, 2 * args.hidden_dim), nn.ReLU(), nn.Linear(2 * args.hidden_dim, args.hidden_dim))
        self.forward_dynamics = nn.Sequential(nn.Linear(
            2 * args.hidden_dim, 2 * args.hidden_dim), nn.ReLU(), nn.Linear(2 * args.hidden_dim, args.hidden_dim))

        self.act_decoder = nn.GRU(
            args.hidden_dim, args.embedding_dim)
        self.act_fc = nn.Linear(args.embedding_dim, vocab_size)

        self.obs_decoder = nn.GRU(
            args.hidden_dim, args.embedding_dim)
        self.obs_fc = nn.Linear(args.embedding_dim, vocab_size)
